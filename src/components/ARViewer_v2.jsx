...
const ARViewer = ({ landmarkId, landmarkName, galleryTypesPath = '/gallery_types.json' }) => {
  // State for gallery data loaded from JSON
  const [galleryData, setGalleryData] = useState(null);
  const [locations, setLocations] = useState([]);
  const [allImages, setAllImages] = useState([]);
  const [isLoadingGallery, setIsLoadingGallery] = useState(true);
  
  // Enhanced tour state (same as FixedVirtualTour)
  ...
  useEffect(() => {
    const loadGalleryData = async () => {
      try {
        ...
        const landmarkKey = Object.keys(data).find(key => 
          data[key].id === landmarkId || key.includes(landmarkId) || data[key].name.toLowerCase().includes(landmarkId.toLowerCase())
        );
        
        if (!landmarkKey) {
          throw new Error(`Landmark ${landmarkId} not found in gallery data`);
        }
        
        const landmarkData = data[landmarkKey];
        
        // Convert JSON data to locations format (same as FixedVirtualTour)
        const convertedLocations = [];
        const imageCategories = landmarkData.images;
        const navPoints = landmarkData.navigationPoints || {};
        const sequentialOrder = landmarkData.sequentialOrder || Object.keys(imageCategories);
        
        sequentialOrder.forEach((category, index) => {
          if (imageCategories[category] && imageCategories[category].length > 0) {
            const nextCategory = sequentialOrder[index + 1] || sequentialOrder[0];
            
            convertedLocations.push({
              id: category,
              name: getCategoryDisplayName(category),
              type: category,
              images: imageCategories[category].map(img => img.src),
              description: `${getCategoryDisplayName(category)} view of ${landmarkData.name}`,
              nextLocation: nextCategory,
              navigationPoints: navPoints[category] || []
            });
          }
        });
        ...
        if (convertedLocations.length > 0) {
          const initialLocation = convertedLocations[0].id;
          setCurrentLocationId(initialLocation);
          setVisitedLocations(new Set([initialLocation]));
        }
  ...
  const getCategoryDisplayName = (category) => {
    const displayNames = {
      ...
      chamber1: 'Main Chamber',
      chamber2: 'Inner Chamber',
  ...
  // All the same functionality as FixedVirtualTour
  ...
  if (isLoadingGallery) {
    return (
      <div className="ar-viewer-loading">
        ...
        <h3>Loading AR Virtual Tour...</h3>
        <p>Preparing {landmarkName} gallery from JSON data</p>
      </div>
    );
  }
  ...
  if (!currentLocation || locations.length === 0) {
    return (
      <div className="ar-viewer-error">
        <h3>⚠️ Gallery Not Found</h3>
        <p>Could not load gallery data for {landmarkName}</p>
        <p>Please check that gallery_types.json exists and contains data for landmark: {landmarkId}</p>
      </div>
    );
  }
    ...
    <div className="ar-viewer">
      ...
      <div className="tour-header">
        <div className="header-left">
          <h2>🏛️ AR Virtual Tour: {viewMode === 'carousel' ? allImages[carouselIndex]?.locationName : currentLocation.name}</h2>
      ...
      <div className="tour-instructions">
        <h4>AR Virtual Tour Navigation (JSON-Powered):</h4>
        <div className="instruction-grid">
          ...
          <div className="instruction-item">
            <strong>📁 JSON-Powered:</strong>
            <ul>
              <li>Images loaded from gallery_types.json</li>
              <li>Supports all image formats (jpg, png, webp)</li>
              <li>Easy to add new landmarks and images</li>
            </ul>
          </div>
        </div>
      </div>
