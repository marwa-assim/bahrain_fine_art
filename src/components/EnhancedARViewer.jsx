...
import './EnhancedARViewer.css';
  ...
  const ARViewer = ({ landmarkId = "2_Bahrain_Fort", galleryTypesPath }) => {
  galleryTypesPath = galleryTypesPath || `${import.meta.env.BASE_URL}complete_gallery_types.json`;
        ...
        locationTypes.forEach((type, index) => {
          if (landmark.images[type] && landmark.images[type].length > 0) {
            const location = {
              ...
              type: type,
              images: landmark.images[type].map(img => `${import.meta.env.BASE_URL}${img.src.replace(/^\//, '')}`),
  ...
  return (
    ...
    <div className="fixed-virtual-tour">
      ...
      {/* Main Image Display */}
      <div className={`images-container ${isAR3DMode ? 'ar-3d-mode' : ''}`}
