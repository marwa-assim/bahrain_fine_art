...
import { galleryImages as allGalleryImages } from '../data/galleryData';
...
const EnhancedGallery = ({ landmark }) => {
  ...
  useEffect(() => {
    const folderKey = Object.keys(allGalleryImages).find(key =>
      key.startsWith(`${landmark.id}_`)
    );

    if (folderKey) {
      const images = allGalleryImages[folderKey].map(filePath => {
        const lowerFile = filePath.toLowerCase();
        const categoryGuess = lowerFile.includes('interior')
          ? 'interior'
          : lowerFile.includes('aerial')
          ? 'aerial'
          : 'exterior';

        return {
          url: `${import.meta.env.BASE_URL}${filePath.replace(/^\//, '')}`,
