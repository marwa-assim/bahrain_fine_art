...
const FixedVirtualTour = ({ landmark, galleryMap }) => {
  // Enhanced tour locations with distributed navigation points
  const [locations, setLocations] = useState([]);

useEffect(() => {
  fetch('/bahrain_fine_art/gallery_types.json')
    .then(res => res.json())
    .then(data => {
      if (!landmark || !landmark.id) return;

      const folderKey = Object.keys(data).find(key => key.startsWith(`${landmark.id}_`));
      if (!folderKey) return;

      const typedImages = data[folderKey];
      const parsed = [{
        id: folderKey,
        name: folderKey.replace(/^\d+_/, '').replace(/_/g, ' '),
        type: 'mixed',
        images: Object.entries(typedImages).flatMap(([type, imgs]) =>
        imgs.map(filename => `${import.meta.env.BASE_URL}gallery/${folderKey}/${filename}`)
),
    ...
    .catch(err => console.error("Failed to load gallery_types.json:", err));
}, [landmark]);
  ...
  const navigateImage = (direction) => {
    const maxIndex = currentLocation?.images?.length ? currentLocation.images.length - 1 : 0;
    if (direction === 'next') {
  ...
  <div className="empty-image-placeholder">No image available</div>
)}
